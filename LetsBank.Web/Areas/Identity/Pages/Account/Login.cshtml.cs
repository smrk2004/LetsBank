        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
					...
					var equivalent_ourUserStore_userId	= StringUtils.StringToGUID(identityUser.Email);

					var ourUserStore_user				=_accountManagementService.GetAccountByUserId(equivalent_ourUserStore_userId);
					if (ourUserStore_user == null)
					{
						// Use Identity's User data -> to generate suitable GuId mapping compatible to our in-memory Account Store
						// NOTE: this same logic is used in both Register.cshtml.cs (& Login.cshtml.cs, here); Our in-memory store has to be recreated
						if (_accountManagementService.CreateAccount(equivalent_ourUserStore_userId))
						{
